...
from chefFusion import utils
from chefFusion import layers


class ChefFusionArgs:
  freeze_lm: bool = True
  freeze_vm: bool = True
  ...
  text_fc_mode: str = 'chefFusion_mapper'
  num_tokens: int = 8
  num_clip_tokens: int = 77


class ChefFusionModel(nn.Module):
  def __init__(self, tokenizer, args: ChefFusionArgs = ChefFusionArgs()):
    super().__init__()
    ...
    embedding_dim = self.input_embeddings.embedding_dim * self.args.n_visual_tokens ##4096 * 4
  ...
  def get_visual_embs(self, pixel_values: torch.FloatTensor, mode: str = 'captioning'):
    if mode not in ['captioning', 'generation']:
      raise ValueError(f"mode should be one of ['captioning', 'generation'], got {mode} instead.")
    ...
    if mode == 'captioning':
      ...
      visual_embs = torch.reshape(visual_embs, (visual_embs.shape[0], self.args.n_visual_tokens, -1))## batch_size, 4, 4096
  ...
  def train(self, mode=True):
    super(ChefFusionModel, self).train(mode=mode)
  ...
  def forward(
    ...
    if mode == 'captioning':
      ...
      print('mode now,', mode)
      print('input_embs dim, ', input_embs.shape)
      print('visual_embs dim, ', visual_embs.shape)
      print('labels dimension, & full_label dim,',labels.shape, full_labels.shape,'\n',labels,full_labels)
      pad_idx = []

      for label in full_labels:
        for k, token in enumerate(label):
          # Mask out gen tokens if they exist.
          if token in [self.tokenizer.pad_token_id] + self.gen_token_idx: 
            label[k:] = -100
            pad_idx.append(k)
            break
      ...
      for label in full_labels:
        for k, token in enumerate(label):
          if (token == self.tokenizer.pad_token_id) or (token in self.gen_token_idx[1:]):
            label[k:] = -100
            break
      output = self.lm(inputs_embeds=input_embs,
                       labels=full_labels,
    ...
    if mode in ['generation']:
      ...
      # Concatenate captions for generation, if specified.
...
#     print('here1')
    with torch.no_grad():  # no tracking history
      ...
      for i in range(max_len):
        ...
        if self.gen_token_idx and self.gen_token_idx[0] != -1:
        ...
        if next_token.shape[0] == 1 and next_token.item() == self.gen_token_idx[0]:
          next_token = torch.tensor(self.gen_token_idx)[None, :].long().to(embeddings.device)  # (1, num_tokens)
        else:
    ...
    print('type in gen', out.device.type, output_embeddings[0].device.type, output_logits[0].device.type)
    return out, output_embeddings, output_logits


class ChefFusion(nn.Module):
  def __init__(self, tokenizer, model_args: Optional[ChefFusionArgs] = None,
               path_array: Optional[List[str]] = None, emb_matrix: Optional[torch.tensor] = None,
               load_sd: bool = False, num_gen_images: int = 1):
    super().__init__()
    self.model = ChefFusionModel(tokenizer, model_args)
  ...
  def generate_for_images_and_texts( 
    self, prompts: List, num_words: int = 0, min_word_tokens: int = 0,  gen_scale_factor: float = 1.0,
    ...
    with torch.no_grad():
      ...
      if num_words == 0:
        ...
        generated_ids, generated_embeddings, _ = self.model.generate(input_embs, num_words, min_word_tokens=min_word_tokens,
          temperature=temperature, top_p=top_p, gen_scale_factor=gen_scale_factor)
      ...
      all_ret_idx = [i for i, x in enumerate(generated_ids[0, :] == self.model.gen_token_idx[0]) if x][:max_num_rets]
      ...
      if len(all_ret_idx) == 0:
        ...
        for ret_idx in all_ret_idx:
          print('first arg: ',generated_ids[0, ret_idx:ret_idx+self.model.num_tokens].cpu().detach().numpy().tolist())
          print('second arg: ', self.model.gen_token_idx)
          assert generated_ids[0, ret_idx:ret_idx+self.model.num_tokens].cpu().detach().numpy().tolist() == self.model.gen_token_idx, (generated_ids[0, ret_idx:ret_idx+self.model.num_tokens], self.model.gen_token_idx)
...
def load_chefFusion(model_dir: str) -> ChefFusion:
 ...
 # embs_paths = [s for s in glob.glob(os.path.join(model_dir, 'cc3m*.npy'))]
  ...
  for i in range(model_kwargs['num_tokens']):
      ...
      gen_token_idx = tokenizer(f'[IMG{i}]', add_special_tokens=False).input_ids
      assert len(gen_token_idx) == 1, gen_token_idx
      model_kwargs['gen_token_idx'].append(gen_token_idx[0])

  args = namedtuple('args', model_kwargs)(**model_kwargs)

  # Initialize model for inference.
  model = ChefFusion(tokenizer, args, path_array=path_array,
               load_sd=True, num_gen_images=1)
  model = model.eval()
  ...
  with torch.no_grad():
      print('shape comparison, ', model.model.input_embeddings.weight[-model_kwargs['num_tokens']:, :].shape, img_token_embeddings.shape)
