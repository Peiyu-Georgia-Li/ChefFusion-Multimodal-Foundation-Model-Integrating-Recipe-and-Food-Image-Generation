...
def get_dataset(args, split: str, tokenizer, precision: str = 'fp32') -> Dataset:
  ...
  if split == 'train':
    if 'recipe1m' in args.dataset:
      dataset_paths.append(os.path.join(args.dataset_dir, 'recipe1m_train.tsv'))
      image_data_dirs.append(os.path.join(args.image_dir, 'recipe1m/training/'))
    else:
      raise NotImplementedError

  elif split == 'val':
    if 'recipe1m' in args.val_dataset:
      dataset_paths.append(os.path.join(args.dataset_dir, 'recipe1m_val.tsv'))
      image_data_dirs.append(os.path.join(args.image_dir, 'recipe1m/validation'))
    else:
      raise NotImplementedError
  ...
  if len(dataset_paths) > 1:
    ...
    dataset = torch.utils.data.ConcatDataset([
      CsvDataset(path, image_dir, tokenizer, 'image',
        ...
        image_size=args.image_size, gen_token_idx=args.gen_token_idx, 
    ...
    print(dataset_paths[0], image_data_dirs[0])
...
class CsvDataset(Dataset):
  def __init__(self, input_filename, base_image_dir, tokenizer, img_key,
               ...
               image_size: int = 224, gen_token_idx: List[int] = [-1],
    ...
    if img_key not in df.columns:
            raise KeyError(f"Column '{img_key}' not found in the DataFrame.")
  ...
  def __getitem__(self, idx):
    while True:
        ##modify here to get to the image
      image_path = self.base_image_dir
      for i in range(0,4):
        image_path = os.path.join(image_path, str(self.images[idx])[i])
      image_path1 = image_path
      image_path = os.path.join(image_path, str(self.images[idx]))
      caption = str(self.captions[idx])
      clip_l_path = os.path.join(image_path1, 'clip_embs', str(self.images[idx]) + '.npy')
