"""Run validation loop for ChefFusion."""
import collections
...
def validate(val_loader, model, tokenizer, criterion, epoch, args):
  ...
  num_words = 500  # Number of words to generate.
  ...
  def run_validate(loader, base_progress=0):
    with torch.no_grad():
      ...
      for i, (image_paths, images, caption_images, _, _, gen_tokens, gen_caption_len, clip_emb) in tqdm.tqdm(enumerate(loader), position=0, total=len(loader)):
        i = base_progress + i
        if torch.cuda.is_available():
        ...
        for model_mode in model_modes:
          ...
          print("input prefix: ", args.input_prompt)
          ...
          if model_mode == 'captioning':
            ...
            print('images.size(0)  && shape at validate.py line 91', images.size(0), images.shape)
            print('loss_item() at validate.py line 91', loss.shape)
            ce_losses.update(loss.sum().item(), images.size(0))#modify loss.item() to loss.sum().item()
          elif model_mode == 'generation':
            if args.num_clip_tokens != args.num_tokens:
              seq_len = clip_emb.shape[1]##77
...
##modified module
            ...
            if args.input_prompt is not None:
              ...
              input_embs = torch.cat([input_embs, prompt_embs], dim=1).to(visual_embs.device) # add to.()
              print('show me device in validate.py line 135, ', input_embs.device, tgt_tokens.device, token_len.device)
              print('show me token_len in validate.py line 136, ', token_len)

            generated_ids, _, _ = model(input_embs, tgt_tokens, token_len, generate=True, num_words=num_words, temperature=0.0, top_p=1.0, min_word_tokens=min_word_tokens)
      ...
      ##add another sacrebleu here as well as rouge here
      sacrebleu_score = sacrebleu.corpus_score(all_generated_captions, full_gt_captions).score
     # sacrebleu_score = sacrebleu.corpus_score(all_generated_captions, full_gt_captions)
      sacreBLEU.update(sacrebleu_score, 1)
      rouge_score_temp = rouge_scorer(all_generated_captions, full_gt_captions)
      rouge1_score =rouge_score_temp['rouge1_fmeasure']
      rouge1.update(rouge1_score, 1)
      rouge2_score =rouge_score_temp['rouge2_fmeasure']
      rouge2.update(rouge2_score, 1)
      rougeL_score =rouge_score_temp['rougeL_fmeasure']
      rougeL.update(rougeL_score, 1)
      ...
      loss = (image_loss + caption_loss) / 2.0
  ...
  sacreBLEU = utils.AverageMeter('sacreBLEU', ':6.2f', utils.Summary.AVERAGE)
  ...
  writer.add_scalar('val/sacreBLEU', sacreBLEU.avg, actual_step)
  writer.add_scalar('val/rouge1', rouge1.avg, actual_step)
  writer.add_scalar('val/rouge2', rouge2.avg, actual_step)
  writer.add_scalar('val/rougeL', rougeL.avg, actual_step)
