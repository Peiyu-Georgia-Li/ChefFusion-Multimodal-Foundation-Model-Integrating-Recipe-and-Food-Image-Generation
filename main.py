...
from chefFusion import data
from chefFusion import losses as losses_utils
from chefFusion import models
from chefFusion import utils
from chefFusion import validate
...
def parse_args(args):
  parser = argparse.ArgumentParser(description='ChefFusion training')
  ...
  parser.add_argument('--gen-loss-scale', type=float, default=1.0, help="Scale on generation loss.")

  parser.add_argument('--concat-captions-prob', type=float, default=0, help="Probability of concatenating two examples sequentially for captioning.")
  ...
  text_fc_modes = ['linear', 'chefFusion_mapper']
  parser.add_argument('--text-fc-mode', default='chefFusion_mapper',
...
def main_worker(gpu, ngpus_per_node, args):
  ...
  # Create model
  model_args = models.ChefFusionArgs()
  ...
  assert args.num_tokens == 0 or 'chefFusion_mapper' in model_args.text_fc_mode or (args.num_tokens * args.gen_emb_dim == args.num_clip_tokens * 768 or args.num_tokens * args.gen_emb_dim == args.num_clip_tokens * 1024), (f'{args.num_tokens} * {args.gen_emb_dim} != {args.num_clip_tokens} * 768 (or 1024)')## here is for checking parameter purpose
  ...
  for i in range(model_args.num_tokens):
    ...
    generation_token_idx = tokenizer(f'[IMG{i}]', add_special_tokens=False).input_ids
    assert len(generation_token_idx) == 1, generation_token_idx
    model_args.gen_token_idx.append(generation_token_idx[0])
    args.gen_token_idx.append(generation_token_idx[0])# adding token index to a list one by one 


  # Save model args to disk.
  with open(os.path.join(args.log_dir, 'model_args.json'), 'w') as f:
    json.dump(vars(model_args), f, indent=4)
  ## freeing GPU Memory: Ensure that you're properly releasing memory by deleting unnecessary tensors and variables when they are no longer needed
  torch.cuda.empty_cache()

  #model = models.load_chefFusion1('/home/path/to/chefFusion/runs/reload_file_name', False, None)
  model = models.ChefFusion(tokenizer, model_args)
...
def train(train_loader, model, tokenizer, criterion, optimizer, epoch, scheduler, args):
  ...
  gen_time = utils.AverageMeter('RetrievalTime', ':6.3f')
  ...
  for i, (_, images, caption_images, _, _, gen_tokens, gen_caption_len, clip_emb) in enumerate(train_loader):
    ...
    model_modes = ['captioning', 'generation']
    ...
    for model_mode in model_modes:
      ...
      if model_mode == 'generation':
        ...
        tgt_tokens, token_len = gen_tokens, gen_caption_len  # For captioning, it doesn't matter.
      ...
      # Measure captioning accuracy for multi-task models.
      ...
      if model_mode == 'generation':#start from here today!!
      ...
      if model_mode == 'generation':
        gen_time.update(time.time() - mode_start)
    ...
    if ((i + 1) % args.grad_accumulation_steps == 0) or (i == args.steps_per_epoch - 1):
      ...
      for param in model.module.model.input_embeddings.parameters():#this is for LLM input embedding
    ...
    with torch.no_grad():
      ...
      for gen_idx in args.gen_token_idx:
        trainable_weight = model.module.model.input_embeddings.weight[gen_idx, :]
        model.module.model.input_embeddings.weight[gen_idx, :].div_(trainable_weight.norm(dim=-1) / frozen_norm)##THIS part is not the normalization to 1 but the frozen_norm

      # Log norms to Tensorboard. !!! could print to see the dimension here
    ...
    if actual_step == 1 or (i + 1) % args.print_freq == 0:
      ...
      if not args.multiprocessing_distributed or (args.multiprocessing_distributed
        ...
        # Append caption text.
        pred_tokens = output[:, args.n_visual_tokens-1:-1, :].argmax(dim=-1)#image token
        generated_captions = tokenizer.batch_decode(pred_tokens, skip_special_tokens=False)

        if model_mode == 'captioning':
          # Truncate before creating images, rather than after. Some compute might be saved.
